function DerivadasGui()
    ...
    dropdown_opciones.ValueChangedFcn = @(src, evt) actualizar_campos();
    btn_calcular.ButtonPushedFcn = @(btn,event) calcular();
    ...
    function actualizar_campos()
        ...
        if strcmp(opcion, 'Derivada numérica personalizada')
    ...
    function calcular()
        ...
        if strcmp(opcion, 'Derivada numérica personalizada')
            ...
            texto_resultados.Value = {
                ...
                'd/dx[cos(x)] = -sin(x)', ...
                'd/dx[tan(x)] = sec^2(x)', ...
                '... y más fórmulas comunes'
            };
        end
    end

    function [da, dc, de] = derivar_funcion(f, x0, h)
        da = (f(x0 + h) - f(x0)) / h;
        dc = (f(x0 + h) - f(x0 - h)) / (2 * h);
        de = (-f(x0 + 2*h) + 8*f(x0 + h) - 8*f(x0 - h) + f(x0 - 2*h)) / (12 * h);
    end
end
